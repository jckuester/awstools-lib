...
func TestInstall_Cache(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			err := os.RemoveAll(testInstallDir)
			require.NoError(t, err)

			p, err := provider.Install(tc.providerName, tc.constraint, testInstallDir)
			require.NoError(t, err)
			...
			p2, err := provider.Install(tc.providerName, tc.constraint, testInstallDir)
			require.NoError(t, err)
...
func TestInstall_PurgeOldVersions(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			p, err := provider.Install(tc.providerName, tc.constraintOld, testInstallDir)
			require.NoError(t, err)
